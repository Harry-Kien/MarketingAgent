...
export interface GatewayDeps {
   ...
   * is the failure mode this field, plus the cold-start gate and the
   * contract-violation check below, exist to close.
   */
 ...
 *
 * Three defences work together here, added across two rounds of review:
 *
 ...
 * 2. (Fix round 2) A cold-start GATE: while this gateway has never yet
 *    observed a real cost (`maxCostSeen === 0`), at most ONE call may be
 *    in flight at a time, regardless of how many are fired concurrently.
 *    `estimatedCostUsd` alone cannot protect a concurrent burst against a
 *    caller who under-declared it (every call in the burst reserves the
 *    same too-low number before any of them can prove it wrong) -- the
 *    gate is what actually bounds first-burst exposure to exactly one call
 *    instead of "however many fit the wrong estimate", which measured as
 *    high as 50 of 50 for an honest-looking but wrong 0.01-vs-0.70
 *    estimate before this fix. The gate releases the instant the gating
 *    call settles (success OR failure) so one slow or failed cold call can
 *    never permanently starve the gateway; once any real, valid cost is
 *    observed, `maxCostSeen` is non-zero forever after and the gate never
 *    applies again for this gateway instance -- ordinary concurrency
 *    (proved by this file's "invokes the provider exactly once..." test,
 *    which always warms the gateway up first) is completely unaffected.
 * 3. (Fix round 2) A CONTRACT check: `estimatedCostUsd` is documented as
...
export function createGateway(deps: GatewayDeps): Gateway {
  ...
  let maxCostSeen = 0;
  let reserved = 0;
  // True while this gateway has never yet observed a real cost AND some
  // call is currently in flight establishing one. Only ever meaningful
  // while `maxCostSeen === 0`; once any real cost lands, every future call
  // skips this gate entirely (see header comment, defence 2).
  let coldGateHeld = false;

  return {
    spentUsd: () => spent,

    async generate(req, ctx) {
      const isCold = maxCostSeen === 0;
      let holdingColdGate = false;

      if (isCold) {
        if (coldGateHeld) {
          const message =
            `Run budget cannot yet be safely judged: this provider's real per-call cost is not ` +
            `known (cold gateway) and another call is already establishing it -- at most one ` +
            `call may be in flight until a real cost is observed.`;
          logger.warn("model call refused: cold-start serialization", {
      ...
      try {
        ...
        // second (warm-gateway) caller see this call's claim on the
        // budget.
        ...
        try {
          ...
          // Fix round 2, CRITICAL, defence 3: estimatedCostUsd is
        ...
        // Released regardless of outcome, so one slow, failed, or invalid
        // cold call can never permanently starve every later attempt.
        if (holdingColdGate) coldGateHeld = false;
      }
    },
  };
}
