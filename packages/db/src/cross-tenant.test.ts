...
describe("E14: cross-table composite foreign keys refuse a cross-workspace parent", () => {
  ...
  it.each(FK_PAIRS)("$childTable -> $parentTable refuses a parent row from a different workspace", async ({ childTable, childCols, parentTable, constraintName }) => {
    ...
    // approval_decision.approval_request_id is UNIQUE (one decision per
    // request), and b.approvalRequestId already carries the fixture's own
    // seeded decision. Hijacking a DIFFERENT column here (e.g. actor_user_id,
    // for the approval_decision->workspace_member pair 0037 added) would
    // otherwise leave approval_request_id pointed at that already-decided
    // request, so the INSERT collides on uniqueness before the FK under test
    // is ever reached. A fresh, decision-less approval_request in B's own
    // workspace sidesteps that without touching the column actually probed
    // (the approval_request_id-hijacking pairs below don't need this: they
    // overwrite that column themselves).
    if (childTable === "approval_decision" && fkColumn !== "approval_request_id") {
      const fresh = await withTenant(pool, b.workspaceId, (tx) =>
        tx.query(
          `insert into approval_request (id, workspace_id, campaign_id, content_version_id, target_channel)
           values ($1, $2, $3, $4, 'meta_page') returning id`,
          [newId(), b.workspaceId, b.campaignId, b.contentVersionId],
        ),
      );
      const freshRequestId = (fresh.rows[0] as { id: string }).id;
      const requestColIndex = row.columns.indexOf("approval_request_id");
      row.cells[requestColIndex] = { value: freshRequestId };
    }
